...
(function($) {
	$.fn.circleDiagram = function(params){
		...
		var text = $('<span class="diagram-text"></span>')
			.css({
				"display": "block",
				"position": "relative",
				"z-index": "2",
				...
				"height": parseFloat(options.size - (options.borderWidth * 2)) + "px",
				"line-height": parseFloat(options.size - (options.borderWidth * 2)) + "px",
				"color": options.textColor
			})
			.text(options.percent)
			.appendTo($this);

		var bg = $('<div class="diagram-bg"></div>')
			.css(cssElems)
			.css({
		...
		var fill = $('<div class="diagram-fill"></div>')
			.css(cssElems)
			.css({
